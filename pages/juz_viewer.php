...
if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0 && $_GET['id'] <= 30) {
    $juz_number = $_GET['id'];
    $juz_data = call_api('juz/' . $juz_number . '/quran-uthmani');
    $ayahs = $juz_data['data']['ayahs'];
} else {
    header("Location: /juzs");
    exit();
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Juz <?php echo $juz_number; ?> - Quran Kareem</title>
...
<body>
    ...
    <main>
        <div class="container py-4">
            <h2 class="mb-4 text-center">Juz <?php echo $juz_number; ?></h2>
            <div class="list-group">
                <?php foreach ($ayahs as $ayah): ?>
                    <div class="list-group-item">
                        <p class="mb-1 ayah-text" dir="rtl"><?php echo $ayah['text']; ?></p>
                        <small class="text-muted"><?php echo $ayah['surah']['name']; ?>:<?php echo $ayah['numberInSurah']; ?></small>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
